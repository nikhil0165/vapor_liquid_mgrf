...
import greens_function_vap_liq
...
def uself_component(n_profile,n_bulk1,n_bulk2,valency,quad_point,domain,epsilon): # integration component of u_long as per legendre-gauss quadrature
    G_full= greens_function_vap_liq.Gcap_full(n_profile,n_bulk1,n_bulk2,valency,quad_point[0],domain,epsilon)
    G_free =  np.ones(len(n_profile))*(1/(2*epsilon*quad_point[0]))## greens_function_vap_liq.Gcap_free(len(n_profile),quad_point[0],domain,epsilon)
    G_component = (G_full-G_free)
    u_component = quad_point[1] * (np.power(valency, 2) / (4 * np.pi)) * G_component[:,np.newaxis]
    return u_component

def uself_long(n_profile,n_bulk1,n_bulk2,valency,domain,epsilon):# long range component of self-energy q**2(G-Go)
    ...
    for i in range(0,floor(quads/cores)):
        with concurrent.futures.ProcessPoolExecutor(max_workers=cores) as executor:
            answer = {executor.submit(uself_component,n_profile,n_bulk1,n_bulk2,valency,quad_point,domain,epsilon): quad_point for quad_point in quad_points[i*cores:(i+1)*cores]}
            for future in concurrent.futures.as_completed(answer):
                u_long = u_long + future.result()
    with concurrent.futures.ProcessPoolExecutor(max_workers=cores) as executor:
            answer = {executor.submit(uself_component,n_profile,n_bulk1,n_bulk2,valency,quad_point,domain,epsilon): quad_point for quad_point in quad_points[floor(quads/cores)*cores:]}
            for future in concurrent.futures.as_completed(answer):
                u_long = u_long + future.result()
    return u_long

def uself_complete(n_profile,n_bulk1,n_bulk2,rad_ions,valency,domain,epsilon): # self energy of all ions
    u_short = uself_short(n_profile,rad_ions,valency,epsilon)
    u_pc = uself_point(n_profile,valency,epsilon)
    u_long = uself_long(n_profile,n_bulk1,n_bulk2,valency,domain,epsilon)
